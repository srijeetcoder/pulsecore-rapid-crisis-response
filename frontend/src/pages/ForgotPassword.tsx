...
export const ForgotPassword = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-void px-4 relative overflow-hidden font-body">
      ...
      <Link 
        to="/" 
        className="absolute top-8 left-8 btn-bitcoin-ghost !px-5 !py-2.5 !text-[10px] font-mono uppercase tracking-[0.2em]"
      >
        <Home className="w-4 h-4 mr-2" />
        Terminal_Home
      </Link>

      <div className="max-w-md w-full space-y-10 card-block !p-10 relative z-10">
        <div className="text-center">
          <div className="relative inline-block mb-6">
            <KeyRound className="h-14 w-14 text-bitcoin-orange mx-auto" />
            <div className="absolute inset-0 bg-bitcoin-orange/20 blur-xl rounded-full animate-pulse"></div>
          </div>
          <h2 className="text-3xl font-heading font-bold text-transparent bg-clip-text bg-gradient-to-r from-bitcoin-orange to-digital-gold tracking-tight uppercase">Reset Protocol</h2>
        ...
        {error && (
          <div className="font-mono text-[10px] text-burnt-orange text-center bg-burnt-orange/10 py-3 rounded-lg border border-burnt-orange/30 uppercase tracking-widest">
            {error}
          </div>
        )}
        {success && (
        ...
        <form className="mt-8 space-y-6" onSubmit={step === 'request' ? handleRequestOtp : handleResetPassword}>
              ...
              <>
                <div>
                  ...
                  <input
                    ...
                    className="input-terminal w-full !text-4xl text-center font-mono tracking-[0.5em] !h-20 text-bitcoin-orange"
                    placeholder="000000"
                    maxLength={6}
                  />
                </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="btn-bitcoin w-full !py-4.5 !text-xs"
          >
        ...
        <div className="text-center">
          <Link to="/login" className="font-mono text-[10px] text-stardust hover:text-bitcoin-orange uppercase tracking-[0.2em] transition-colors">
